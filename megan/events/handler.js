...
class EventHandler {
    ...
    // Handle group participants update - FIXED for new config structure
    async handleGroupUpdate(update) {
        ...
        if (update.action === 'add') {
            // Get welcome message from database or config (new structure)
            const welcomeMsg = this.bot.db?.getSetting('welcome_msg', config.WELCOME?.MESSAGE || '👋 Welcome to the group!');
            ...
            // Get goodbye message from database or config (new structure)
            const goodbyeMsg = this.bot.db?.getSetting('goodbye_msg', config.GOODBYE?.MESSAGE || '👋 Goodbye!');
