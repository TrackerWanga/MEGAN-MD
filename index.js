...
class MeganBot {
    ...
    constructor() {
        ...
        this.ownerHelper = new OwnerHelper(config, this.db);
    ...
    async sendStartupMessage() {
        ...
        try {
            ...
            // Get owner JID from ownerHelper

            const ownerJid = this.ownerHelper.getOwnerJid();

            

            // If no owner set yet, don't send startup message

            if (!ownerJid) {

                this.logger.info('No owner set yet - skipping startup message');
            ...
            const ownerNumber = this.ownerHelper.getOwnerNumber() || 'Not set';
            ...
            this.logger.success(`Startup message sent to owner: ${ownerNumber}`);
        ...
        try {
            ...
            sock.ev.on('connection.update', (update) => {
                ...
                if (connection === 'open') {
                    ...
                    // ===== NO MORE setOwnerFromConnection =====

                    // Owner comes from .env or first login now

                    

                    // Update mode from database

                    this.ownerHelper.getMode().then(mode => {

                        this.logger.success(`Connected! Mode: ${mode}`, '✅');
