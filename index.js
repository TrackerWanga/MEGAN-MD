    ...
    async cleanupSessionFiles() {
        try {
            ...
            if (otherJsonFiles.length > 0) {
                ...
                // Filter out failed stats and sort by newest first
                const validStats = fileStats.filter(f => f !== null);
                validStats.sort((a, b) => b.mtime - a.mtime);
                
                // Keep only the 5 newest files (plus creds.json = 6 total)
                const newestFiles = validStats.slice(0, 5).map(f => f.file);
                filesToKeep.push(...newestFiles);
                
                // Delete ALL other files (sender-key-*, session-*, app-state-*, pre-key-*, etc.)
            ...
            // Also clean up any non-JSON files that might be old
            ...
            // Completely silent - no error logging for cleanup
        }
    }
