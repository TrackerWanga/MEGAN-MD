...
class MeganBot {
    ...
    constructor() {
        ...
        // Initialize Session Manager (will be set after connection)
    ...
    // ===== AUTO-CREATE ALL REQUIRED FOLDERS FOR PTERODACTYL =====
    ...
    createRequiredFolders() {
        ...
        folders.forEach(folder => {
            ...
            try {
                ...
                this.logger.debug(`📁 Folder ensured: ${folder}`);

            } catch (error) {

                this.logger.error(`Failed to create folder ${folder}: ${error.message}`);
    ...
    async initialize() {
        ...
        try {
            ...
            console.log('║     𝐌𝐄𝐆𝐀𝐍-𝐌𝐃 𝐌𝐔𝐋𝐓𝐈-𝐔𝐒𝐄𝐑       ║');

            console.log('╚════════════════════════════════════╝');

            console.log('');

            // Session from .env (MAIN BOT)
            ...
            // Load all commands (for main bot)

            await this.loadCommands();

            // Initialize Session Manager for multi-user support

            this.sessionManager = new SessionManager(this, 3); // Max 3 users

            this.logger.success('Session Manager initialized (max 3 users)', '👥');
    ...
    async setupSession() {
        ...
        await fs.writeJson(path.join(mainSessionDir, 'creds.json'), sessionData, { spaces: 2 });

        

        // Update config.SESSION_DIR to point to main folder for main bot

        config.SESSION_DIR = mainSessionDir;

        

        this.logger.success('Main session ready');
    ...
    async sendStartupMessage() {
        ...
        try {
            ...
            const activeSessions = this.sessionManager ? this.sessionManager.getActiveCount() : 0;
            ...
            const message = `✅ *MEGAN BOT CONNECTED*\n\n` +
                ...
                `👥 *User Sessions:* ${activeSessions}/3\n` +
        ...
        try {
            ...
            sock.ev.on('connection.update', (update) => {
                ...
                if (connection === 'open') {
                    ...
                    this.logger.success('Main bot connected!');

                    setTimeout(() => this.sendStartupMessage(), 2000);

                } else if (connection === 'close') {

                    this.logger.warn('Main bot connection closed. Reconnecting...');
