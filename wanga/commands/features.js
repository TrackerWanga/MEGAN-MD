...
commands.push({
    ...
    async execute({ msg, from, sender, args, bot, sock, react, reply }) {
        if (args.length === 0) {
            ...
            await react('ℹ️');
            return reply(`🗑️ *Anti-Delete Settings*\n\nCurrent: ${status}\n\nOptions: on, off\n\nWhen ON, deleted messages are sent to owner DM.`);
        }
...
commands.push({
    ...
    async execute({ msg, from, sender, args, bot, sock, react, reply }) {
        const helpText = `┏━━━━━━━━━━━━━━━━━━━┓\n` +
                        ...
                        `• ${config.PREFIX}anticall <false/reject/block>\n` +
                        `• ${config.PREFIX}antilink <false/delete/warn/kick>\n` +
                        `• ${config.PREFIX}autoreact <on/off>\n` +
                        `• ${config.PREFIX}autoread <on/off>\n` +
                        `• ${config.PREFIX}presence [inbox/group] [type]\n` +
                        `• ${config.PREFIX}setmsg [type] [message]\n\n` +
                        `━━━━━━━━━━━━━━━━━━━\n` +
                        `> created by wanga`;
