...
// TOGGLE FEATURE COMMANDS - OPEN TO EVERYONE
...
commands.push({
    ...
    description: 'Toggle anti-delete feature (false/inbox/indm/both)',
    ...
    async execute({ msg, from, sender, args, bot, sock, react, reply }) {
        ...
        if (args.length === 0) {
            ...
            const current = await bot.db.getSetting('antidelete', 'false');

            await react('ℹ️');

            return reply(`🔰 *Anti-Delete Settings*\n\nCurrent: ${current}\n\nOptions: false, inbox, indm, both`);

        }

        const option = args[0].toLowerCase();

        if (!['false', 'inbox', 'indm', 'both'].includes(option)) {

            await react('❌');

            return reply('❌ Invalid option! Use: false, inbox, indm, both');
        ...
        await reply(`✅ Anti-delete set to: ${option}`);
...
commands.push({
    ...
    async execute({ msg, from, sender, args, bot, sock, react, reply }) {
        ...
        if (args.length < 2) {
            ...
            return reply(`👤 *Presence Settings*\n\nUsage: .presence [inbox/group] [online/typing/recording/offline]\n\nExample: .presence inbox typing`);
...
commands.push({
    ...
    async execute({ msg, from, sender, args, bot, sock, react, reply }) {
        ...
        if (args.length < 2) {
            ...
            return reply(`📝 *Set Messages*\n\nUsage: .setmsg [type] [message]\n\nTypes: anticall, antilink, welcome, goodbye\n\nExample: .setmsg anticall Calls are not allowed!`);
